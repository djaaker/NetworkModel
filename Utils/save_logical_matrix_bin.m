function save_logical_matrix_bin(filename, matrix, mode)
    % SAVE_LOGICAL_MATRIX_BIN Saves or appends a logical 2D matrix to a binary file.
    % 
    % Inputs:
    %   filename - String, name of the binary file (without extension)
    %   matrix   - 2D logical matrix to be saved/appended
    ...
    if mode == 'w'
        ...
        fwrite(fid, uint8(matrix), 'uint8'); % Store as uint8 (1 byte per value)
        fclose(fid);
    ...
    elseif mode == 'a'
        ...
        fwrite(fid, uint8(matrix), 'uint8'); % Append in uint8 format
        fclose(fid);
