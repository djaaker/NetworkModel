function [LFP] = calculateLFP(SNN,v_neurons,ge_neurons,gi_neurons)
    ...
    LFP.lfp = abs(circshift(ge_neurons.*(E_e-v_neurons),[0 tau_shift]) - 1.65*(gi_neurons.*(E_i-v_neurons)));
    ...
    if row_remainder > 0, LFP.LFP_combined(end,1:N_blocks,:) = LFP.LFP_row; end
    if col_remainder > 0, LFP.LFP_combined(1:N_blocks,end,:) = LFP.LFP_col; end
    if row_remainder > 0 && col_remainder > 0, LFP.LFP_combined(end,end,:) = LFP.LFP_corner; end
